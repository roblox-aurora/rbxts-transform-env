...
export function formatTransformerDebug(message: string, node?: ts.Node): string {
	if (node) {
		...
		const str = `${chalk.gray("[rbxts-transform-env]")} ${chalk.green("macro debug")} ${chalk.cyan(
			info.relativePath,
		)}:${chalk.yellow(info.linePos)} - ${message}\n${chalk.italic(node.getText())}`;
		return str;
	} else {
		return `${chalk.gray("[rbxts-transform-env]")} ${chalk.green("macro debug")} - ` + message;
	}
}

export function formatTransformerInfo(message: string, node?: ts.Node): string {
	if (node) {
		const str = `${chalk.gray("[rbxts-transform-env]")} ${chalk.cyan("macro info")} - ${message}\n${chalk.italic(
			node.getText(),
		)}`;
		return str;
	} else {
		return `${chalk.gray("[rbxts-transform-env]")} ${chalk.cyan("macro info")} ` + message;
	}
}
...
export function formatTransformerWarning(message: string, node?: ts.Node, suggestion?: string): string {
	if (node) {
		const info = getDebugInfo(node);
		let str = `${chalk.gray("[rbxts-transform-env]")} ${chalk.yellow("macro warning")} ${chalk.cyan(
			info.relativePath,
		)}:${chalk.yellow(info.linePos)} - ${message}\n${chalk.italic(node.getText())}`;
		...
		return `${chalk.gray("[rbxts-transform-env]")} ${chalk.yellow("macro warning")} - ` + message;
	}
}
...
export function formatTransformerDiagnostic(message: string, node?: ts.Node, suggestion?: string): string {
	if (node) {
		const info = getDebugInfo(node);
		let str = `${chalk.gray("[rbxts-transform-env]")} ${chalk.red("macro error")} ${chalk.cyan(
			info.relativePath,
		)}:${chalk.yellow(info.linePos)} - ${message}\n${chalk.italic(node.getText())}`;
		...
		return `${chalk.gray("[rbxts-transform-env]")} ${chalk.red("macro error")} - ` + message;
	}
}
